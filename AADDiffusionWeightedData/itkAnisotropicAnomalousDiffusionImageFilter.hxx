...
{
    return  GeneralizedDiffCurve()*exp((-1.0)*pow(static_cast<double>(abs((idxValue - centerValue)))/static_cast<double>(m_Conductance),  (double)(2.0)));
}
...
::GeneralizedDiffCurve()
{
    ...
    if (m_Q < 1.0) {
        ...
        d = m_GeneralizedDiffusion * exp((-1.0) * (pow((double)(m_Q - 1.0), (double)(2.0))) / (double)(0.08));
