    ...
    for(int i=0; i<m_Iterations; i++){
        ...
        while( !auxIt.IsAtEnd() )
        {
            if (auxIt.Get()!=static_cast<InputPixelType>(0)) {
                ...
                for (unsigned int idx = 0; idx < pow((double)(laplaceIt.GetSize()[0]),static_cast< double >(OutputImageDimension)); ++idx) {
                    neighborAux += pow((double)(laplaceIt.GetPixel(idx)), (double)(2.0 - m_Q))*laplaceOp.GetElement(idx);
                }
